...
[SELF-TEST v1]
Based on: 本文档「测试用例」一节 + 同目录 `PRD-18607.ac.json`（design 棒产出，13 条用例，经人工 groom 从 5b 推回 5c 后由本棒执行）
Repo: https://github.com/MSPbotsAI/voipms-mcp@dev01.mbagent/PRD-18607 · 起点 main@eb772ac · 代码位置：已 push
...
|---|---|
| 命令 | `uv run pytest` |
| 本次（final） | passed=19（exit=0） |
| 基线（baseline） | 交付前仓库只有 README 脚手架，无 `pyproject.toml`/无测试框架，基线=0 个测试（`tdd.py run --label baseline` 记为 counts unparsed，是全新交付的预期形状，不是解析失败） |
| 新增失败（回归） | 无 —— `tdd.py diff --from baseline --to final`：REGRESSIONS none / progress none / still failing none |

（上表用管道分隔但不带 `|---|`，是本文档的一致排版，不是照抄 ClickUp 排版禁忌——git 里正常按 GFM 表格渲染。）

### 用例矩阵

| 用例 | AC | 类型 | 状态 | 测试 / 证据 |
|---|---|---|---|---|
| TC1 | AC1 | 自动 | 绿 | `tests/test_tools.py::test_tc1_find_subaccount_match` |
| TC2 | AC1 | 自动 | 绿 | `tests/test_tools.py::test_tc2_find_subaccount_no_match_returns_empty_not_error` |
| TC3 | AC2 | 自动 | 绿 | `tests/test_tools.py::test_tc3_get_subaccount_includes_current_voicemail` |
| TC4 | AC2 | 自动 | 绿 | `tests/test_tools.py::test_tc4_get_subaccount_not_found` |
| TC5 | AC3 | 自动 | 绿 | `tests/test_tools.py::test_tc5_list_voicemail_boxes_returns_entries` |
| TC6 | AC3 | 自动 | 绿 | `tests/test_tools.py::test_tc6_list_voicemail_boxes_empty_is_not_an_error` |
| TC7 | AC4 | 自动 | 绿 | `tests/test_tools.py::test_tc7_set_voicemail_redirect_reports_before_after` |
| TC8 | AC4 | 自动 | 绿 | `tests/test_tools.py::test_tc8_set_voicemail_redirect_preserves_other_fields`（断言发给 vendor 的 `setSubAccount` 请求体里 description/callerid_number/account 与改动前一致） |
| TC9 | AC5 | 自动 | 绿 | `tests/test_tools.py::test_tc9_invalid_target_rejected_without_vendor_write`（断言 mock 的 `setSubAccount` handler 未被调用） |
| TC10 | AC6 | 自动 | 绿 | `tests/test_tools.py::test_tc10_vendor_unauthorized_maps_and_hides_credentials`（断言返回文本里不含测试凭据字面量） |
| TC11 | AC7 | 自动 | 绿 | `tests/test_tools.py::test_tc11_upstream_5xx_exhausts_retries_then_upstream_error`（断言恰好重试 3 次后返回 upstream_error/retryable=true） |
| TC12 | AC8 | 手工 | 未执行 | 依赖两套真实/沙箱 VOIP.ms 执行账号凭据，客户尚未提供。已用单测验证了请求级隔离机制本身（`tests/test_middleware.py::test_context_var_is_reset_after_request`：ContextVar 在 `finally` 里被 reset），但并发多租户的端到端场景仍需真实凭据 |
| TC13 | AC9 | 手工 | 未执行 | 依赖 `mcp-service-gateway/vendor/registry.yaml` 登记（跨仓库，另计，未完成）。已本地验证：带 Header、不做 `initialize` 握手直接 `tools/list`（验证 stateless_http 生效）返回全部 4 个 `voipms_` 前缀工具及正确的 readOnly/destructive/idempotent 标注 |

未通过/未说明的用例：无（TC12/TC13 是「未执行」而非「未通过」，原因见上表）。

### 改动清单

全部为新增（全新交付，仓库交付前只有一份 `README.md`，无现有实现可改）：

- `pyproject.toml` / `uv.lock` / `.python-version` [新增] —— uv 项目配置，`mcp[cli]>=1.9.0,<2.0.0`（实测解析到 1.30.0）、httpx、pydantic-settings、uvicorn、starlette
- `.gitignore` / `.dockerignore` [新增]
- `Dockerfile` / `docker-compose.yml` [新增] —— 严格照抄工单附件 `vendor-mcp-development-sop 1.md` §8 的参考模板
- `src/voipms_mcp/config.py` [新增] —— 非凭据配置，`SettingsConfigDict(extra="ignore")`
- `src/voipms_mcp/api_client.py` [新增] —— `VoipmsClient`，httpx 异步封装，5s/30s 超时，429/5xx 有限重试（3 次）+ 指数 backoff，非 2xx/status!=success 统一抛 `VoipmsApiError(status_code, message)`
- `src/voipms_mcp/errors.py` [新增] —— 统一错误信封 `{"error":{"code","message","retryable"}}` + `status_code → code` 映射
- `src/voipms_mcp/server.py` [新增] —— FastMCP 工厂（`stateless_http=True`、关闭 DNS-rebinding 保护）+ `CredentialMiddleware`（`contextvars.ContextVar`，`finally` reset）+ `/health`
- `src/voipms_mcp/tools/subaccounts.py` [新增] —— 4 个工具：`voipms_find_subaccount` / `voipms_get_subaccount` / `voipms_list_voicemail_boxes` / `voipms_set_voicemail_redirect`
- `src/voipms_mcp/__main__.py` [新增] —— uvicorn 入口
- `README.md` [改] —— 补齐 Header 凭据表格（SOP §0.3/§3.1 要求）、本地运行说明、实现状态
- `tests/test_tools.py` / `tests/test_middleware.py` / `tests/conftest.py` [新增] —— 见上方用例矩阵

**vendor API 方法名核实**（技术设计 Open Questions 里标注"未联网核对"的那一条，本棒已核实）：对照公开的 `python-voipms` 参考实现（<https://github.com/4doom4/python-voipms>，`voipms/entities/accountsget.py` / `accountsset.py` / `voicemailget.py`）确认 `getSubAccounts`（可选 `account` 过滤）、`getVoicemails`（可选 `mailbox` 过滤）为真实方法名；**并确认 `setSubAccount` 确实要求整份记录**（`id`/`password`/`auth_type`/`device_type`/`lock_international`/`international_route`/`music_on_hold`/`allowed_codecs`/`dtmf_mode`/`nat` 均为必填），印证了本单"必须 read-modify-write、绝不能单字段覆写"的前提是真实存在的风险而非过度设计。未对真实 VOIP.ms 账号发起过请求，字段的运行时行为仍待 SOP §10 的真实联调验证。

### 锁现状的测试（改动前后都绿）

不适用 —— 全新仓库，交付前只有 `README.md`，没有需要锁定的现有行为。

### 被改过的用例

无。TC1–TC13 的判定句（`text`/`expect`）原样保留自 design 棒产出的 `PRD-18607.ac.json`，本棒只补充/纠正了 `type: auto` 用例的 `tests` 字段（测试函数名——执行方式，属于本棒的自由，不是判定内容）。
...
- `uv run pytest` → exit 0（19 passed，含 2 条超出 ac.json 13 条用例范围的额外覆盖：`test_tools_list_snapshot` 校验 4 个工具的 schema/annotations、`test_not_configured_when_no_credentials` 校验缺凭据时的 `not_configured` 分支）
- 回归判定：`tdd.py diff --from baseline --to final` 新增失败 0 条
- 注意：这是全新仓库，尚无 CI 配置；本地 `uv run pytest` 是当前唯一门禁，`tdd.py detect` 的判定一致（`declared_commands: []`，落到通用 Python 门禁）

### 本地验证的已知盲区（如实记录，不计入"通过"）

- **真实 VOIP.ms 端到端联调（SOP §10 强制项）未做**：客户尚未提供 API 开通/执行账号/API 密码/出口 IP 白名单（技术设计"风险与依赖 · 依赖 1"）。已完成的本地验证：`uv run python -m voipms_mcp` 起服务后，`GET /health` 返回 200；缺 Header 时 `POST /mcp` 返回 401 并在 body 里列出缺失的 Header 名；带任意（非真实）Header 值调用 `tools/list`（不做 `initialize` 握手，验证 `stateless_http=True` 确实生效，未退回 SOP §1.3 提到的那个 400 坑）返回全部 4 个工具及正确的 `readOnlyHint`/`destructiveHint`/`idempotentHint` 标注。
- **Docker 镜像未验证可构建**：本机 Docker daemon 未运行（`docker version` 报 `failed to connect to the docker API at npipe:////./pipe/docker_engine`），环境阻塞，非代码问题。`Dockerfile` 严格照抄 SOP §8 的参考模板（多阶段构建、非 root 用户、装 curl、`HEALTHCHECK` 打 `/health`、`EXPOSE 8080`）。
- TC12（多租户隔离）、TC13（Library 部署可见性）：手工用例，均"未执行"，原因见上方用例矩阵。

## 交接

[交接 v1]
基线分支：main @ eb772ac（`voipms-mcp` 是全新仓库，只有 `main` 一条分支，没有 `release`/`release-aks`）
工作分支：dev01.mbagent/PRD-18607（已推送到远端，未删除）
用了哪些绕过开关：无 —— 这个仓库没有 mspack，也没有 `--no-migrate`/`--no-check` 这类开关

## 门禁与用例（S5 自测结果，逐条见「自测」一节）

门禁：全过（`uv run pytest` exit 0，19 passed，0 回归）
用例：13 条，已绿 11 条（TC1–TC11，全自动），未执行 2 条（TC12/TC13，手工，均因客户凭据/跨仓库登记尚未就绪，见「自测」一节）

## 给接手人的提醒

- 全新仓库，无 schema、无数据库迁移、无既有用户/既有功能，因此没有"改坏别的功能"的回归面。
- **本仓库不走本 skill 常规的"INT → 上线分支 → mspack 打包发布"路径**：按工单 comment 的人工指示（Kaka Zhang，2026-09-09）"仓库已经新好,只有main分支,最终不用合并进INT,直接合进main就行,不用打包"，只有一条 `main` 分支，合并目标就是 `main`，且不需要打包发布——这与其它 mb-platform-* 仓库的发布路径不同，人工合入时请勿套用 mspack 那一套流程。
- 合入 `main` 前建议至少补两件事（本棒因环境/凭据限制未能完成，均已如实记录在「自测」一节的"本地验证的已知盲区"）：① 一次真实的 `docker build --platform linux/amd64 -t voipms-mcp:dev .`；② 客户凭据到位后的一次真实 `tools/call` 全链路联调（SOP §10 强制的手工端到端）。
- 合入 `main` 后，这个连接器要在产品 Library 里出现还需要**另一次跨仓库改动**：在 `mcp-service-gateway` 的 `vendor/registry.yaml` 追加 `voipms` 条目（`repo_url: https://github.com/MSPbotsAI/voipms-mcp.git`，凭据字段见本仓 README 的 Header 表格）。不在本单范围内，见技术设计"风险与依赖 · 依赖 2"。
- 交付说明自查（vendor-mcp-development-sop.md §14 Checklist）：对接契约（§1/§3：/mcp 与 /health、stateless、Header 凭据、请求级隔离、DNS-rebinding 关闭、lifespan 挂到外层 app）、错误与网络（§4/§5：结构化错误信封、有限重试、超时）、面向 Agent 设计（§2：工具数、description 长度、命名前缀、语义标注）三类均已在代码里落实并有单测覆盖。容器与交付物（§0.3/§8）里"docker build 可构建并正常启动"一项因本机无 Docker daemon 未能自查，请接手人在具备条件的机器上补跑一次。
